...
namespace TaCertoForms.Factory {
    public class PessoaProfessorCreator : BaseCreator, IFactoryPessoa {
        ...
        public List<Pessoa> PessoaList() {
            ...
            List<int> turmaAlunoIds = new List<int>();
            ...
            foreach(var tda in tda_aux) {
                ...
                List<TurmaAluno> ta_aux = db.TurmaAluno.Where(ta => ta.IdTurma == t.IdTurma && !turmaAlunoIds.Contains(ta.IdTurmaAluno)).ToList();
                turmaAlunos = turmaAlunos.Concat(ta_aux).ToList();
                foreach (var i in ta_aux)
                    turmaAlunoIds.Add(i.IdTurmaAluno);
            }
